const Show = ({ task, setTask, history, setHistory }) => {
  ...
  return (
    <div className="col-sm-10 col-lg-6 m-auto display">
      <div className="row justify-content-center bg-light pb-2 border border-secondary border-3 rounded shadow">
        ...
        {
          data.length==0 && (
            <div className="col-12 rounded text-center bg-complete h4 p-4">Either you have finished all the tasks or You have not added any task</div>
          )
        }
        {task != undefined && (
          <>
            {data.map((d) => (
              <div className="col-12 text-left list py-2 mt-1" key={d}>
                ...
                <span className="pl-5 list-content text-center">{task[d]}</span>
              </div>
            ))}
          </>
        )}
